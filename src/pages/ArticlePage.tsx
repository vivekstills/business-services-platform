...
function CategoryBadge({ cat }: { cat: string }) {
  ...
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[calc(11px+3pt)] font-semibold border ${cls}`}>{cat}</span>;
}
  ...
  return (
    <Link to={`/articles/${article.slug}`}
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[calc(13px+3pt)] font-semibold text-gray-800 group-hover:text-blue-700 transition-colors leading-snug line-clamp-2">{article.title}</p>
        <p className="text-[calc(11px+3pt)] text-gray-400 mt-1 flex items-center gap-1"><Clock className="w-3 h-3" /> {article.readingTime}</p>
      </div>
    </Link>
  );
}
      ...
      <div className="relative bg-transparent overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[calc(11px+3pt)] text-gray-400 flex items-center gap-1"><Clock className="w-3 h-3" /> {article.readingTime}</span>
          </div>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.05 }}
            className="text-gray-500 text-[calc(15px+3pt)] leading-relaxed mb-6 max-w-2xl">
            {article.excerpt}
          </motion.p>

          <div className="flex flex-wrap items-center gap-4 text-[calc(12px+3pt)] text-gray-400">
            ...
            {article.tags.length > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                {article.tags.map((tag) => (
                  <span key={tag} className="px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 text-[calc(11px+3pt)]">{tag}</span>
                ))}
              </div>
            )}
          </div>
                ...
                {prev ? (
                  <Link to={`/articles/${prev.slug}`}
                    ...
                    <div>
                      ...
                      <p className="text-[calc(13px+3pt)] font-semibold text-gray-700 group-hover:text-blue-700 leading-snug line-clamp-2">{prev.title}</p>
                    </div>
                  </Link>
                ) : <div />}
                {next && (
                  <Link to={`/articles/${next.slug}`}
                    ...
                    <div>
                      ...
                      <p className="text-[calc(13px+3pt)] font-semibold text-gray-700 group-hover:text-blue-700 leading-snug line-clamp-2">{next.title}</p>
                    </div>
                  </Link>
                )}
              </div>
          ...
          <aside className="space-y-5">
            ...
            {sidebar.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-200/60 p-5">
                ...
                <Link to="/articles" className="mt-3 flex items-center justify-center gap-1.5 text-[calc(12px+3pt)] font-semibold text-blue-600 hover:text-blue-800 transition-colors pt-3 border-t border-gray-100">
