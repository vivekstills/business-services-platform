...
export default function ContactUsPage() {
  ...
  const inputCls = (key: string) =>
    `w-full h-[46px] rounded-xl bg-white border px-4 text-[14px] text-gray-800 placeholder-gray-300 focus:outline-none transition-all ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Name *</label>
            ...
            <div>
              <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Email *</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Phone *</label>
              <div className="relative">
            ...
            <div>
              <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Message (optional)</label>
              <div className="relative">
                <MessageSquare className="absolute left-3 top-3.5 w-4 h-4 text-gray-300" />
                <textarea value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={3} className="w-full rounded-xl bg-white border border-gray-200 pl-10 pr-4 py-3 text-[14px] text-gray-800 placeholder-gray-300 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 resize-none" placeholder="How can we help?" />
              </div>
            </div>
            ...
            {isSubmitted && <p className="text-[13px] text-emerald-600">Thank you! We'll get back to you within 24 hours.</p>}
          </form>
        </div>
      </div>
    </PolicyLayout>
