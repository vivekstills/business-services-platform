...
function FAQItem({ faq, index, openIndex, setOpenIndex }: {
  ...
  return (
    ...
    >
      ...
      >
        <span className={`text-[calc(14px+1.5pt)] font-medium leading-snug transition-colors ${isOpen ? 'text-blue-800' : 'text-gray-700'}`}>
          {faq.q}
        </span>
        <ChevronDown
        ...
        {isOpen && (
          ...
          >
            <div className="px-5 pb-5 pt-0">
              ...
              <p className="text-[calc(13.5px+1.5pt)] text-gray-500 leading-relaxed">{faq.a}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          >
            ...
            <div className="flex items-center gap-2.5 mb-5">
              <div className="flex items-center gap-2 bg-blue-600 text-white px-3.5 py-1.5 rounded-full">
                ...
                <span className="text-[calc(12px+1.5pt)] font-bold">{selectedState}</span>
              </div>
              <h3 className="text-[calc(16px+1.5pt)] font-bold text-gray-800">
                State-specific FAQs
              </h3>
            </div>
              ...
              {stateFAQs.map((faq, idx) => (
                ...
                >
                  <button
                    ...
                    className="w-full flex items-start justify-between gap-4 px-5 py-4 text-left"
                  >
                    <span className={`text-[calc(14px+1.5pt)] font-medium leading-snug transition-colors ${stateOpen === idx ? 'text-blue-800' : 'text-gray-700'}`}>
                      {faq.q}
                    </span>
                    <ChevronDown
      ...
      <AnimatePresence>
        {selectedState && stateFAQs.length === 0 && faqs.length > 0 && (
          ...
          >
            ...
            <p className="text-[calc(12.5px+1.5pt)] text-gray-500">
