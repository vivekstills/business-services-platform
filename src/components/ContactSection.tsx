...
export default function ContactSection() {
  ...
  const inputCls = (key: string) =>
    `w-full h-[46px] bg-white border rounded-xl pl-10 pr-4 text-[calc(14px+2pt)] text-gray-800 placeholder-gray-300 focus:outline-none transition-all shadow-sm ${
      errors[key]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div>
            ...
            <motion.p
              ...
              className="text-gray-500 text-[calc(15px+2pt)] leading-relaxed mb-10"
            >
              {cs.subheading}
            </motion.p>
            ...
            <div className="space-y-5">
              {cs.features.map((item, i) => {
                ...
                <div key={i} className="flex items-start gap-4 p-4 rounded-xl bg-white/70 backdrop-blur-sm border border-gray-100/80 shadow-sm">
                  ...
                  <div>
                    <div className="text-[calc(14px+2pt)] font-semibold text-gray-800">{item.label}</div>
                    <div className="text-[calc(13px+2pt)] text-gray-500 mt-0.5">{item.desc}</div>
                  </div>
                </div>
              );
              })}
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Name */}
              <div>
                <label className="block text-[calc(12px+2pt)] font-semibold text-gray-500 uppercase tracking-wider mb-2">Full Name *</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[calc(12px+2pt)] font-semibold text-gray-500 uppercase tracking-wider mb-2">Email *</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-[calc(12px+2pt)] font-semibold text-gray-500 uppercase tracking-wider mb-2">Phone *</label>
                  <div className="relative">
              ...
              {/* Service */}
              <div>
                <label className="block text-[calc(12px+2pt)] font-semibold text-gray-500 uppercase tracking-wider mb-2">Service Required *</label>
                <select
                  {...field('service')}
                  className={`w-full h-[46px] bg-white border rounded-xl px-4 text-[calc(14px+2pt)] text-gray-700 focus:outline-none transition-all appearance-none cursor-pointer shadow-sm ${
                ...
                {errors.service && <p className="text-[calc(11px+2pt)] text-red-500 mt-1">{errors.service}</p>}
              </div>

              {/* Message */}
              <div>
                <label className="block text-[calc(12px+2pt)] font-semibold text-gray-500 uppercase tracking-wider mb-2">Message (optional)</label>
                <div className="relative">
                  <MessageSquare className="absolute left-3 top-3.5 w-4 h-4 text-gray-300" />
                  <textarea
                    {...field('message')}
                    rows={3}
                    className="w-full bg-white border border-gray-200 rounded-xl pl-10 pr-4 py-3 text-[calc(14px+2pt)] text-gray-700 placeholder-gray-300 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all resize-none shadow-sm"
              ...
              {isSubmitted && (
                <div className="flex items-start gap-2 bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                  ...
                  <span className="text-[calc(12.5px+2pt)] text-emerald-700">
