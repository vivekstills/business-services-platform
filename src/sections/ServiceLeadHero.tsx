...
export default function ServiceLeadHero({ service }: Props) {
  ...
  const inputCls = (key: string) =>
    `w-full h-[44px] rounded-xl bg-white border px-3 pl-9 text-[calc(13.5px+1.5pt)] text-gray-800 placeholder-gray-300 focus:outline-none transition-all shadow-sm ${
      errors[key]
  ...
  const selectCls = (key: string) =>
    `w-full h-[44px] rounded-xl bg-white border pl-9 pr-4 text-[calc(13.5px+1.5pt)] text-gray-700 focus:outline-none transition-all shadow-sm appearance-none cursor-pointer ${
      errors[key]
        ? 'border-red-300 focus:border-red-400 focus:ring-2 focus:ring-red-100'
        : 'border-gray-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100'
    }`;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <div>
              ...
              <motion.p
                ...
                className="text-gray-500 text-[calc(15px+1.5pt)] leading-relaxed mb-8"
              >
              ...
              <motion.div
                ...
                className="space-y-3"
              >
                <p className="text-[calc(11px+1.5pt)] font-bold uppercase tracking-widest text-gray-400 mb-4">How it works</p>
                {steps.map((step, idx) => (
                  <div key={idx} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-[calc(11px+1.5pt)] font-bold text-blue-700 flex-shrink-0 mt-0.5">
                      {idx + 1}
                    </div>
                    <span className="text-[calc(13.5px+1.5pt)] text-gray-600 leading-snug">{step}</span>
                  </div>
                ))}
              </motion.div>
            </div>
            ...
            >
              <div className="bg-white/90 backdrop-blur-xl border border-white/60 ring-1 ring-gray-200/50 rounded-2xl p-7 shadow-2xl shadow-blue-100/20">
                <p className="text-[calc(11px+1.5pt)] font-bold uppercase tracking-widest text-gray-400 mb-1">Apply for</p>
                <h2 className="text-xl font-bold text-gray-900 mb-6">{service.name}</h2>

                <form onSubmit={handleSubmit} className="space-y-4">
                  {/* Name */}
                  <div>
                    <label className="block text-[calc(12px+1.5pt)] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">
                      Name
                    </label>
                  ...
                  <div>
                    ...
                    {errors.state && <p className="text-[calc(11px+1.5pt)] text-red-500 mt-1">{errors.state}</p>}
                      ...
                      {hint && (
                        ...
                        >
                          <div className="mt-2 flex items-start gap-2 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
                            ...
                            <p className="text-[calc(11.5px+1.5pt)] text-blue-700 leading-snug">{hint}</p>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  ...
                  {submitError && (
                    <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-xl p-3">
                      ...
                      <span className="text-[calc(12px+1.5pt)] text-red-600">{submitError}</span>
                    </div>
                  )}
                  {isSubmitted && (
                    <div className="flex items-start gap-2 bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                      <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
                      <span className="text-[calc(12px+1.5pt)] text-emerald-700">
                        Confirmation sent to your email & phone. Our team will contact you shortly.
                      </span>
                    </div>
                  )}

                  <p className="text-[calc(11px+1.5pt)] text-gray-400 leading-relaxed">
        ...
        <section className="relative bg-gradient-to-b from-white via-slate-50/50 to-white py-16 lg:py-20 overflow-hidden noise-overlay">
          ...
          <div className="relative z-[2] max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div className="max-w-2xl mb-12">
              ...
              <p className="text-gray-500 text-[calc(15px+1.5pt)]">
                {hasCustomTabbedPricing
            ...
            {hasCustomTabbedPricing && customPricingTabs[Math.min(serviceTabIdx, customPricingTabs.length - 1)]?.description?.trim() && (
              <p className="text-gray-600 text-[calc(14px+1.5pt)] mb-8 max-w-3xl">
            ...
            <AnimatePresence>
              {selectedState && statePackages && (
                ...
                >
                  ...
                  <p className="text-[calc(12.5px+1.5pt)] text-blue-700 font-medium">
                      ...
                      >
                        {showBadgePill && (
                          <div className="absolute -top-3.5 left-6">
                            <span className={`px-3 py-1 rounded-full text-[calc(11px+1.5pt)] font-bold shadow ${badgeClass}`}>
                              {badgeLabel}
                            </span>
                          </div>
                        )}
                        ...
                        <div className="mb-6">
                          ...
                          <p className={`text-[calc(13px+1.5pt)] ${isGradient ? 'text-blue-200' : 'text-gray-400'}`}>
                            {pkg.description}
                          </p>
                        </div>
                        ...
                        {pkg.features && pkg.features.length > 0 && (
                          <div className="flex-1 mb-8">
                            <p className={`text-[calc(10.5px+1.5pt)] font-bold uppercase tracking-[0.15em] mb-3 ${isGradient ? 'text-blue-300' : 'text-gray-400'}`}>
                              What's included:
                            </p>
                            <ul className="space-y-3">
                              {pkg.features.map((f, fi) => (
                                <li key={fi} className={`flex items-start gap-2.5 text-[calc(13.5px+1.5pt)] ${isGradient ? 'text-blue-100' : 'text-gray-600'}`}>
                        ...
                        <div className="mt-auto space-y-2.5">
                          {amountPaise !== null && (
                            <button
                              ...
                              className={`w-full flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-[calc(13.5px+1.5pt)] font-semibold transition-all ${
                                isGradient
                          ...
                          >
                            {showBadgePill && (
                              <div className="absolute -top-3 left-5">
                                <span className={`px-2.5 py-0.5 rounded-full text-[calc(10px+1.5pt)] font-bold shadow ${badgeClass}`}>
                                  {badgeLabel}
                                </span>
                              </div>
                            )}
                            <div className="flex items-start justify-between gap-3 mb-4">
                              <div>
                                ...
                                {pkg.description && (
                                  <p className={`text-[calc(12px+1.5pt)] mt-1 ${isGradient ? 'text-blue-200' : 'text-gray-400'}`}>
                                    {pkg.description}
                                  </p>
                                )}
                              </div>
                            ...
                            {pkg.features && pkg.features.length > 0 && (
                              <ul className="space-y-2 mb-4 flex-1">
                                {pkg.features.map((f, fi) => (
                                  <li key={fi} className={`flex items-start gap-2 text-[calc(12.5px+1.5pt)] ${isGradient ? 'text-blue-100' : 'text-gray-600'}`}>
                            ...
                            <div className="space-y-2 mt-auto">
                              {amountPaise !== null && (
                                <button
                                  ...
                                  className={`w-full flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-[calc(13px+1.5pt)] font-semibold transition-all ${
                                    isGradient
