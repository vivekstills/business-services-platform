...
const t = fs.readFileSync(path.join(__dirname, 'batch6-extracted.txt'), 'utf8').replace(/---PAGE \d+---\r?\n/g, '');
const markers = [];
for (let n = 1; n <= 9; n++) {
  const idx = t.indexOf(`ARTICLE ${n} OF 9 |`);
  if (idx === -1) {
...
const outDir = path.join(__dirname, 'batch6-raw');
